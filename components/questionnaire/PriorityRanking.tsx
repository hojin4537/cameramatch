...
import { PriorityOrder, PriorityCriteria } from "@/lib/types";

interface PriorityRankingProps {
  priorityOrder: PriorityOrder;
  onChange: (order: PriorityOrder) => void;
}

const priorityItems: { key: PriorityCriteria; label: string }[] = [
  { key: "weight", label: "무게" },
  { key: "size", label: "작은 크기" },
  { key: "sharpness", label: "화질" },
...
export default function PriorityRanking({
  ...
  const [draggedItem, setDraggedItem] = useState<PriorityCriteria | null>(null);
  const [dragOverItem, setDragOverItem] = useState<PriorityCriteria | null>(null);

  const moveUp = (key: PriorityCriteria) => {
    ...
    const swapKey = Object.keys(priorityOrder).find(
      (k) => priorityOrder[k as PriorityCriteria] === currentRank - 1
    ) as PriorityCriteria;
  ...
  const moveDown = (key: PriorityCriteria) => {
    const currentRank = priorityOrder[key];
  ...
  const handleDragStart = (e: React.DragEvent, key: PriorityCriteria) => {
    setDraggedItem(key);
    e.dataTransfer.effectAllowed = "move";
    e.dataTransfer.setData("text/html", "");
  };

  const handleDragOver = (e: React.DragEvent, key: PriorityCriteria) => {
    e.preventDefault();
  ...
  const handleDrop = (e: React.DragEvent, targetKey: PriorityCriteria) => {
    e.preventDefault();
